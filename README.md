...
:exclamation: 请注意：全角引号 `“”` 和 `‘’` 后也需要增加空格。
...
:exclamation: 段落内的省略号必须使用全形省略号，`shift` + `6`

:exclamation: 例外：在特殊场景下如`敬请期待...`时，允许使用三个半角句号`...`
...
:exclamation: 请勿出现`（练习）`字样

### 使用 code 或 pre 标签

当存在多行完整语句时，只使用 code 或 pre 高亮需要用户输入的语句，其他语句不高亮显示。例如：

错误：

> 我们也可以使用 ```int i1, i2;``` 来同时声明两个变量，但现在还是使用两条语句吧！请输入：```int i1; int i2;```

正确：

> 我们也可以使用 int i1, i2; 来同时声明两个变量，但现在还是使用两条语句吧！请输入：```int i1; int i2;```

当存在至少一条完整语句时，只使用 code 或 pre 高亮需要用户需要输入的完整语句，描述中出现的变量名、关键字、值等不高亮显示。例如：

错误：

> 我们要声明 `i1` 和 `i2` 两个变量，请输入：```int i1, i2;```

正确：

> 我们要声明 i1 和 i2 两个变量，请输入：```int i1, i2;```

:exclamation: 两点考虑，一是如果无论是否需要输入的内容都进行高亮，用户很容易分不清哪条语句才是真正需要输入的；二是 code 和 pre 在一个段落中出现次数过多会导致段落异常难看
